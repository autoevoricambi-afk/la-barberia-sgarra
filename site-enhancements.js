(function enhanceSgarraSite() {
  ...
  function installVisualRestyle() {
    ...
    style.textContent = `
      ...
      .booking-staff-box{margin:0 0 1rem;padding:.9rem;border:1px solid var(--line-strong);background:rgba(178,138,70,.055)}
      .booking-staff-box label{display:grid;gap:.4rem;font-weight:800;color:var(--cream)}
      .booking-staff-box select{min-height:50px;padding:.65rem .75rem;border:1px solid var(--line-strong);background:#0b0d0c;color:var(--cream);font:inherit}
      .booking-staff-help{margin:.5rem 0 0;color:var(--muted);font-size:.82rem;line-height:1.45}
      .booking-capacity-note{display:flex;align-items:flex-start;gap:.55rem;margin:.65rem 0 0;color:var(--brass-soft);font-size:.8rem}
  ...
  function installStaffPicker() {
    ...
    box.innerHTML = `
      ...
      <p class="booking-staff-help">Paolo e Giuseppe hanno due agende indipendenti. Se scegli “Primo disponibile”, vedrai anche quante postazioni restano libere nello stesso orario.</p>
      <p class="booking-capacity-note"><span class="booking-capacity-dot" aria-hidden="true"></span><span>Slot ogni 30 minuti · capacità massima 2 clienti nello stesso orario, uno per barbiere.</span></p>
    `;
  ...
  function updateBookingLead() {
    ...
    lead.textContent = slug === 'any'
      ? 'Scegli il primo posto libero tra Paolo e Giuseppe: lo stesso orario resta prenotabile finché almeno uno dei due è disponibile.'
