...
async function getStaffState(request, response, context) {
  ...
  try {
    ...
    return sendJson(response, 200, { ok: true, staff: staff || [], hours: hours || [], assignments: appointments || [], blocks: blocks || [] });
  } catch (error) {
...
async function reassignAppointment(body, admin, response, context) {
  ...
  try {
    const result = await supabaseRequest('/rest/v1/rpc/admin_reassign_appointment', {
      method: 'POST',
      body: { p_appointment_id: appointmentId, p_staff_slug: staffSlug, p_actor_id: admin.id, p_reason: normalizeText(body?.reason, 300) || null }
    });
    const appointment = Array.isArray(result) ? result[0] : result;
    try { await processPendingOutboxForReference(appointment?.reference); } catch (notificationError) { logError(context, 'admin_reassign_notification_deferred', notificationError); }
...
async function saveStaffSchedules(body, admin, response, context) {
  ...
  if (!staffHours.length) return sendJson(response, 400, { ok: false, error: { code: 'invalid_settings', message: 'Mancano gli orari degli operatori.' } });
  ...
  try {
    for (const schedule of staffHours) {
      ...
      if (!isValidBookingId(staffSlug) || !allowed.has(staffSlug)) return sendJson(response, 400, { ok: false, error: { code: 'invalid_staff', message: 'Operatore non valido.' } });
      const validation = validateBookingSettingsPayload({ services: body.services, hours: schedule.hours, location: body.location });
      if (!validation.ok) return sendJson(response, 400, { ok: false, error: { code: 'invalid_settings', message: `Controlla servizi e orari di ${staffSlug}.`, fields: validation.errors } });
      const result = await supabaseRequest('/rest/v1/rpc/admin_replace_booking_settings', {
        method: 'POST',
        body: {
          p_staff_slug: staffSlug,
          p_services: validation.value.services.map((item) => ({ slug: item.slug, name: item.name, description: item.description, duration_minutes: item.durationMinutes, buffer_before_minutes: item.bufferBeforeMinutes, buffer_after_minutes: item.bufferAfterMinutes, price_cents: item.priceCents, active: item.active, sort_order: item.sortOrder })),
          p_hours: validation.value.hours.map((item) => ({ weekday: item.weekday, opens_at: item.opensAt, closes_at: item.closesAt, active: item.active })),
          p_location: { min_notice_minutes: validation.value.location.minNoticeMinutes, booking_horizon_days: validation.value.location.bookingHorizonDays, slot_interval_minutes: validation.value.location.slotIntervalMinutes, public_booking_enabled: validation.value.location.publicBookingEnabled, review_url: validation.value.location.reviewUrl, cancellation_strike_limit: validation.value.location.cancellationStrikeLimit, deposit_amount_cents: validation.value.location.depositAmountCents, deposit_payment_url: validation.value.location.depositPaymentUrl },
          p_actor_id: admin.id
        }
      });
...
export default async function handler(request, response) {
  ...
  let body;
  try { body = await readJsonBody(request); } catch { return sendJson(response, 400, { ok: false, error: { code: 'invalid_json', message: 'Richiesta non valida.' } }); }
